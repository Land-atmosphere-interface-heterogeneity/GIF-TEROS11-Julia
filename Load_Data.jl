...
function loadmet(path::AbstractString)
        ...
        [download("http://www.atmos.anl.gov/ANLMET/numeric/2020/"*i*"20met.data", joinpath(path, i*"20met.data")) for i in ["jan", "feb"]];
	...
	[push!(metdata, CSV.read(joinpath(path, i*"20met.data"), delim=' ', header=col_name, ignorerepeated=true, datarow=1, footerskip=2)) for i in ["jan", "feb"]]
...
function PrecipD(metdata::DataFrame, Dtime_met::Array{DateTime,1})
	Precip_d = Array{Float64}(undef, 121)
	Dtime_met_d = Array{DateTime}(undef, 121)
	for i = 1:30
