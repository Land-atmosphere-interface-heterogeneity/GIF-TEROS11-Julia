...
surface!(scene3D_1, 0:7, 0:7, elev, color = lift(X-> GLMakie.vec2color(Matrix(sparse(x, y, SWC_daily[X,:])), Reverse(:lighttest), (0.35, 0.48)), sl.value), shading = false, limits = Rect(0, 0, 0, 7, 7, 1.5));
...
cbar[1] = layout[2, 2:7] = LColorbar(scene, height = 20, limits = (0.35, 0.48), label = "SWC", colormap = :lighttest, vertical = false, labelpadding = -5);
...
surface!(scene3D_2, 0:7, 0:7, elev, color = lift(X-> GLMakie.vec2color(Matrix(sparse(x, y, Tsoil_daily[X,:])), :lighttest, (1, 7)), sl.value), shading = false, limits = Rect(0, 0, 0, 7, 7, 1.5));
x_or = [0,0,0]; Ylen = 7; Zlen = 1; Xlen = 7;
mesh!(scene3D_2, lift(X-> HyperRectangle(Vec3f0(x_or), Vec3f0(Xlen, Ylen, Wtable_daily[X])), sl.value) , color = RGBAf0(0,0,1,0.2));	
cbar[2] = layout[2, 10:15] = LColorbar(scene, height = 20, limits = (1, 7), label = "Tsoil", colormap = :lighttest, vertical = false, labelpadding = -5);
