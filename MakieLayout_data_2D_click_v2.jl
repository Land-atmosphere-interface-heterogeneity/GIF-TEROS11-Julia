...
lines!(ax[1], lift(X-> Point2f0[(X, 0), (X, 60)], xpos));
...
lines!(ax[3], lift(X-> Point2f0[(X, 0), (X, 7)], xpos));
...
barplot!(ax[10], 1:96, lift(X-> Data.Precip_c[1+(X-1)*96:X*96], xpos), color = :blue, strokewidth = 2, strokecolor = :black);
